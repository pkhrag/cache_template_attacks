...
//  FILE* addrfile = fopen("addr.txt", "r");
 // unsigned char *i = 0;
  //fscanf(addrfile, "%p", &i);
  for(size_t i=0; i < range; i+=64)
  {
   printf("%s,%8p",filename,(void*)offset + (long long int)i);
  printf("%s,%8p",filename,i);
  for (j = 0; j < chars_len; ++j)
   {
     key[0] = chars[j];
     for (size_t k = 0; k < 5; ++k)
       sched_yield();
     nkeys = 0;
     flush(start + (long long int)i);
     for (size_t k = 0; k < 5; ++k)
       sched_yield();
     count = flushandreload(start + (long long int)i, duration);
     printf(",=%4ld/%4ld",count,nkeys);
    }
    printf("\n");
    if (1000 * (long long int)i / range > promille)
    {
      promille = 1000 * (long long int)i / range;
